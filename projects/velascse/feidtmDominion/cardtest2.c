...
int main()
{
    int i;
    int seed = 5;
    int k[10] = { adventurer, gardens, embargo, village, council_room, mine, cutpurse,
    ...
    assertTrue(before, after); // Replaces village

    printf("\nChecking deck count to make sure it is 1 less card\n");
    before = stateOG.deckCount[0];
    after = state.deckCount[0];
    assertTrue(before-1, after); // Took 3 cards from players pile

    // Checking Actions
    printf("\nChecking to see if action is plus 2\n");
    before = stateOG.numActions;
    after = state.numActions;
    assertTrue(before+2, after); // Replaces village
